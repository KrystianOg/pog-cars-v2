...
export const useFeatureDiscovery = <T extends HTMLElement>(key: FeatureDiscoveryKey) => {
  const ref = useRef<T | null>(null)
  const rectRef = useRef<DOMRect | null>(null)

  const [value, setValue] = useLocalStorage<boolean>(`feature-discovery-${key}`, false)
...
export const FeatureDiscovery = ({children, discoveryKey, ...props}: FeatureDiscoveryProps) => {
  const [childRef, rectRef, {value, setValue}]= useFeatureDiscovery<HTMLDivElement>(discoveryKey)
  ...
  return (
    <div className={styles['feature-discovery']}>
      <div {...props} className={classnames(!value && styles['feature-discovery'])} />
